...
void App::ImGUI()
{
	...
	if (ImGui::BeginMainMenuBar())
	{
		...
		if (ImGui::BeginMenu("Options"))
		{
			...
			//ImGui::MenuItem("Pause Physics", "Space-Bar", &PausePhysics);
			ImGui::Checkbox("Pause Physics", &PausePhysics);

			ImGui::SeparatorText("Graphics");
			//if (ImGui::CollapsingHeader("Graphics", ImGuiTreeNodeFlags_Leaf));
			...
			/*if (ImGui::BeginMenu("V-Sync"))
			{
				static bool vsync = false;

				if (ImGui::MenuItem("On", "", vsync == true))
				{
					app.setVerticalSyncEnabled(true);

					vsync = true;
				}
				if (ImGui::MenuItem("Off", "", vsync == false))
				{
			...
			{
				if (ImGui::MenuItem("Dark", "", darkLightMode == false))
				{
					darkLightMode = false;

					updateThemeColors();
				}
				if (ImGui::MenuItem("Light", "", darkLightMode == true))
				{
					darkLightMode = true;
		...
		if (ImGui::BeginMenu("Help"))
		{
			...
			if (ImGui::BeginMenu("Controls"))
			{
				...
				{
					static const std::string opt[4][2] = { //HardCode
					...
					for (int row = 0; row < 4; row++) //HardCode
					{
			...
			if (ImGui::BeginMenu("Contacts"))
			{
				...
				{
					{ "Open Github Repo", "https://github.com/IrfanJames/Physics-Simulator" },
					{ "Copy Email ", "irfanjamespak@gmail.com" }
				};

				for (int i = 0; i < 2; i++)
				{
					if (i == 0 && ImGui::Button(links[i][0].c_str()))
					{
						ShellExecute(0, 0, L"https://github.com/IrfanJames/Physics-Simulator", 0, 0, SW_SHOW);
						sf::Clipboard::setString(links[i][1]);
					}

					if (i == 1 && ImGui::Button(links[i][0].c_str()))
					{
						sf::Clipboard::setString(links[i][1]);
					}
